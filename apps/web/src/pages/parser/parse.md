...
`options` に渡せるのは次の 2 つです。

| オプション          | 型                  | 既定     | 内容                                                                                                                                |
| :------------------ | :------------------ | :------- | :---------------------------------------------------------------------------------------------------------------------------------- |
| `extensions`        | `Extension[]`       | なし     | 記法の拡張。[記法の拡張](/parser/extend/)を参照                                                                                     |
| `tableCellNotation` | `"link"` \| `"all"` | `"link"` | テーブルのセルの中で読む記法。`"link"` は Cosense Web と同じくリンクの記法だけ、`"all"` は行と同じくすべての記法 (拡張の記法も含む) |
